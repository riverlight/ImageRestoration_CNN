...
class NewIRNet5(nn.Module):
    def __init__(self):
        super(NewIRNet5, self).__init__()
        self.chl_mid = 32
        self.convDW9x9 = BlockDW2(3, self.chl_mid, 9)
        self.dn2 = BlockDN(self.chl_mid, self.chl_mid, 3, 4)
        self.dn3 = BlockDN(self.chl_mid*2, self.chl_mid, 3, 8)
        self.dn4 = BlockDN(self.chl_mid*3, self.chl_mid, 3, 12)
        self.blockOut = BlockOut(self.chl_mid*4, 3, 3, 16)
    ...
    def forward(self, x):
        ...
        dn4_out = self.dn4(out)
        out = t.cat([dw_out, dn2_out, dn3_out, dn4_out], 1)
...
def test():
    ...
    net = NewIRNet5().to(device)
    outputs = net(inputs)
    print(outputs.shape)
    t.save(net, 'd:/nir5_test.pth')
